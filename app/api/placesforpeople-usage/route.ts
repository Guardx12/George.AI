...
const BUSINESS_SLUG =
  process.env.PLACESFORPEOPLE_USAGE_BUSINESS ||
  'placesforpeople'
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return Response.json(
        { error: details || 'Could not forward Places for People usage to Zapier.' },
    ...
    return Response.json({ error: 'Could not log Places for People usage right now.' }, { status: 500 })
  }
}
